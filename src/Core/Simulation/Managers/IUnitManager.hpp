...
namespace sw::core
{
	...
	public:
		...
		virtual std::vector<Unit*> getUnitsInRadius(const Position& center, uint32_t radius) const = 0;
		virtual std::function<Unit*(UnitId)> getUnitFunction() const = 0;
		virtual std::function<std::vector<Unit*>(const Position&, uint32_t)> getUnitsInRadiusFunction() const = 0;
