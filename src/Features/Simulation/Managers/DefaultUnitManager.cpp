...
namespace sw::features
{
	...
	std::vector<sw::core::Unit*> DefaultUnitManager::getUnitsInRadius(
		const sw::core::Position& center, uint32_t radius) const
	{
		...
		for (const auto& unit : units)
		{
			...
			sw::core::Position unitPos = unit->getPosition();
			uint32_t distance = center.chebyshevDistance(unitPos);
			if (distance <= radius)
			{
	...
	std::function<std::vector<sw::core::Unit*>(const sw::core::Position&, uint32_t)>
	DefaultUnitManager::getUnitsInRadiusFunction() const
	{
		return [this](const sw::core::Position& center, uint32_t radius) -> std::vector<sw::core::Unit*>
		{
