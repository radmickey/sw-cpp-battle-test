...
int main(int argc, char** argv)
{
	...
	parser.add<io::SpawnSwordsman>(
		[&](auto command)
		{
			auto unit = factory.create<features::Swordsman>(
				command.unitId, core::Position{command.x, command.y}, command.hp, command.strength);
	...
	parser.add<io::March>(
		[&](auto command)
		{
			...
			core::Position currentPos = unit->getPosition();
			simulator.setUnitTargetPosition(command.unitId, core::Position{command.targetX, command.targetY});
			eventLog.log(
				commandTick,
				io::MarchStarted{command.unitId, currentPos.x, currentPos.y, command.targetX, command.targetY});
		});
